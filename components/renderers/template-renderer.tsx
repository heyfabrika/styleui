...
const TemplateRenderer = ({
    ...
    const [packageManager, setPackageManager] = useState<"bun" | "npm" | "pnpm" | "yarn">("bun");
    ...
    const currentCommand = installationCommands.find(cmd => cmd.name === packageManager) || installationCommands[0];
    ...
    if (compact) {
        return (
            <Link href={href ?? src} className="group flex flex-col gap-3">
                <div
                    className="w-full aspect-[16/10] bg-muted/20 dark:bg-muted/40 border border-border/50 relative overflow-hidden rounded-xl"
                >
                    {previewImage && (
                        <Image
                            src={previewImage}
